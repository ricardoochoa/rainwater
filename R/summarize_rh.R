...
summarize_rh <- function(x, year = "2020"){
  ...
  n <- nrow(x) / length(calendar)
  x$date = rep(calendar, n) # add a column of the dates of the year to the x parameter data frame.
  ...
  if(n>1){
    TEMP <- stats::aggregate(cbind(x$usedrw, x$demand)~x$tank.size, FUN = sum)
    names(TEMP) <- c("tank.size", "supply", "demand")
    TEMP$percentage <- 100 * TEMP$supply / TEMP$demand
    return(TEMP)
  }else{
